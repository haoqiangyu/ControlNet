...
if __name__ == '__main__':
    ...
    input_image = cv2.imread("./test_imgs/%s"%filename)
    #图片调整到长边800
    if input_image.shape[0] > input_image.shape[1]:
        input_image = cv2.resize(input_image, (int(input_image.shape[1] * 512 / input_image.shape[0]), 512))
    else:
        input_image = cv2.resize(input_image, (512, int(input_image.shape[0] * 512 / input_image.shape[1])))
    ...
    for im in output:
        #写出图片
        i += 1
        cv2.imwrite("./output/output_%s-%s.jpg"%(prompt, i), im)